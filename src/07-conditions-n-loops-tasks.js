...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  let d = b;
  if (a > b) [d, c] = [a, b];
  const start = isStartIncluded ? `[${c}, ${d}` : `(${c}, ${d}`;
  return isEndIncluded ? `${start}]` : `${start})`;
}
...
function isCreditCardNumber(ccn) {
  ...
  const getNum = (num) => {
    const sum = (digits) => digits.toString(10).split('').reduce((acc, el) => (+acc) + (+el), 0);
    if (sum(num) > 9) return sum(sum(num));
    return sum(num);
  };

  const sum = ccnArray.reduce((acc, el, i) => {
    let result = acc;
    if (i % 2 !== 0) {
      result += el * 2 > 9 ? getNum(el * 2) : el * 2;
    } else {
      result += el * 1;
    }
    return result;
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (Object.keys(brackets).indexOf(current) > -1) {
...
const getCommonDirectoryPath = (pathes) => {
  ...
  const arr = parts[0].reduce((acc, item, i) => {
    if (parts.every((part) => part[i] === item)) acc.push(item);
    return acc;
  }, []);
